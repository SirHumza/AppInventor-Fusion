...
<body>
    ...
    <script>
            ...
            const projectHtml = `
                <div class="project-card" id="${projectId}">
                    ...
                    <label class="file-input-label d-block text-center mb-3">
                        ...
                        <input type="file" class="d-none" accept=".aia" 
                               onchange="loadProject(event, '${projectId}')">
                    </label>
        ...
        async function loadProject(event, projectId) {
            ...
            zip.forEach((relativePath, file) => {
                if (relativePath.endsWith('.scm')) {
                    project.screens.push(relativePath.split('/').pop().replace('.scm', ''));
                } else if (relativePath.startsWith('assets/')) {
                    project.assets.push(relativePath.split('/').pop());
                }
            });
        ...
        async function mergeProjects() {
            ...
            // Afegir el fitxer `youngandroidproject/project.properties`
...
`;
            ...
            for (const project of projects) {
                ...
                const zip = await JSZip.loadAsync(projectDiv.querySelector('input[type="file"]').files[0]);
            ...
            // Descarregar el projecte fusionat
        ...
        // Afegir el primer projecte per defecte
        addProject();
    </script>
</body>
</html>
